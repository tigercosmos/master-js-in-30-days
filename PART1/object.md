...
關於物件，更常使用的情境是單純作為資料儲存的容器，就如同之前在介紹陣列時一樣。

如果只拿物件來存資料，就只會使用到上面介紹物件中的屬性了。在這邊的情境下，物件不會有任何方法。
...
這是另外一種 `for` 迴圈的寫法，可以列舉物件中的 key。在每一次的迴圈裡面，`key` 都代表不同的值。
...
物件也可以不僅包含屬性，還能有方法，此外物件方法可以呼叫自己的屬性，方法也可以呼叫方法。如果要呼叫自己的東西就要用 `this` 關鍵字代表自己，如果從外面呼叫的話，就是直接 `物件名字.屬性` 或是 `物件名字.方法()`。
