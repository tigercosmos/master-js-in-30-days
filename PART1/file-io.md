...
資料傳輸在寫程式的時候，不管是你想要從文件中讀取資料，或是資料從網路上傳輸，亦或是資料在程式與程式之間傳送，都常常會用到。事實上我們寫程式通常也就是為了處理資料，如果沒有資料的話，還真的不曉得程式乘坐什麼事情！（有啦，算算質數之類的 XD）
...
與檔案相關的操作，在 NodeJS 中是 [`fs`](https://nodejs.org/api/fs.html) 模組[^3]，亦即檔案系統（file system）。
...
假設你今天想要把 `file1` 搬過去 `file2`，如果直接用檔案方式來處理，那程式會先將 `file1` 檔案的內容一次全部讀進去記憶體中，待讀完之後，再一次寫進 `file2` 檔案中。這時候會發生先前提到的問題，就是整個程序就被這個 I/O 給卡死了。
...
我們已經認識資料在電腦中會有哪幾種型態了，我們在 NodeJS 要處理資料的時候，其實就是用剛剛提到的 `fs`、`stream` 和 `Buffer`來做操作。不過在開始講檔案操作之前，必須先瞭解 NodeJS 模組中的同步執行（synchronous process）和異步執行（asynchronous process）[^4]。這邊簡單介紹，在之後會有更深入的討論。
...
不過異步行為模式其實和其他語言非常不同，像是 C++、Python 等之類的程式語言，所有函式都是同步的，所以都會是從上到下一行一行執行。（當然他們也能寫異步，確切來說是這些語言的官方標準函式庫都是同步的）
...
第一種，在 NodeJS 的內建模組中，只要原本是設計成異步的函數，有些會有對應的同步版本。比方說 `fs.stat`，他原本是異步的，你必須用回傳函數來操作他，但有另外一個 `fs.statSync` 函數，他就是同步的函數，就不會有回傳函數在裡面，像是：
...
當然你的 UID 可能會是其他數字。假設你這邊不是用 `fs.statSync` 而是用 `fs.stat` 的話，就會噴錯：
...
這是因為程式碼直接繼續往下走了，但他要讀取的 `stats` 還沒得到東西（還在跑），於是就出錯了。
